...
import { EndTripForm, EmergencyReportForm, DriverExpenseForm } from '../components/driver';
...
const TripPage = () => {
  ...
  const [showExpenseForm, setShowExpenseForm] = useState(false); // Add state for expense form
  ...
  return (
    <div className="overflow-hidden flex flex-col h-full">
      {/* End Trip, Emergency, and Expense buttons */}
      <div className="p-4 flex justify-between">
        ...
        <button
          onClick={handleExpenseFormClick}
      ...
      {showExpenseForm && (
        <div className="fixed top-0 left-0 right-0 bottom-0 flex items-center justify-center bg-gray-800 bg-opacity-50">
          <div className="bg-white p-8 rounded-lg shadow-lg relative">
            <DriverExpenseForm trip={trip} />
            <button onClick={() => setShowExpenseForm(false)} className="absolute top-2 right-2 text-gray-500 hover:text-gray-800">
              <FaTimes />
            </button>
          </div>
        </div>
